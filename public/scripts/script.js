...
const coffeeAudio = [
  new Audio(
    "https://nichtrijder.pepijnblom.com/sounds/krijg%20nou%20een%20bakkie%20pleur.mp3"
  ),
...
function playCoffee() {
  const randomIndex = Math.floor(
    Math.random() * coffeeAudio.length
  );
...
function setDispencerClass(drink) {
  dispenser.forEach((dispenserSelector) => {
    const dispenserElement = document.querySelector(
      dispenserSelector
    );
    dispenserElement.classList.add("liquid", drink);
    setTimeout(
      () =>
        dispenserElement.classList.remove("liquid", drink),
      12000
    );
  });
}
